...
prob_train = transtab.predict(model, df_alin_train.iloc[:, 1:], df_alin_train.iloc[:, 0])
result_train = get_final_result(threshold,prob_train, df_alin_train.iloc[:, 0])
dict_result_obtain(threshold, prob_train, np.array(df_alin_train.iloc[:,0]),
                   '/home/hci/QYang/YQ_LiverFailure/transtab-main/Real_Prediction/dict_'+model_name+'_train_result.json')
'''
